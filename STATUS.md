...
| `POST /api/sw_autofocus/continuous {x,y,threshold,check_ms,...}` | 연속 AF(초기 합초 후 모니터, 선명도 하락 시 재합초). 상태는 SSE(af_continuous). `/cancel`로 정지 |
...
- ✅ **직사각형 ROI(박스 드래그)** — `focus_measure`를 roi_w/roi_h로 일반화(정사각형은 roi_w=roi_h 호환). UI: 무장 후 라이브뷰 **드래그=박스 ROI**, 거의 안 움직이면 점-선택. 딥러닝 디텍터 바운딩박스(#4)를 받을 인터페이스 토대. **하드웨어 검증**(A7C, mean_luma 57): 디포커스 후 박스 ROI(0.6×0.2) AF→coarse 피크 8.3배(8→66) 회복. ⏳ UI 드래그 시각 검증은 브라우저에서.
- ⚠️ **SW-AF fine<coarse 이슈(발견, 미해결)** — 좁고 뾰족한 coarse 피크(예: 63~66)에서 fine이 더 낮은 위치(15)에 멈춤. **fine 범위 보정 적용**(swaf_lock: `fine_n ≥ 2·⌈step/fine_step⌉+2`로 coarse 격자 덮음) — 무해하나 이 케이스는 **못 고침** → 원인은 fine 범위가 아니라 **백래시**(coarse best로 Near 복귀 시 상대 스텝 유격)가 유력. 절대 위치 API 없는 상대 MF의 본질 한계. 깨끗한 재현은 조명/초점 드리프트로 어려움. 다음 후보: (a) coarse도 복귀를 한 방향(Far) 접근으로 백래시 보정, (b) frames↑로 노이즈 완화, (c) fine best < coarse best면 coarse 유지(단 노이즈 스파이크 시 역효과)
