...
- **붉은 야간 모드** — 🌙 토글로 UI 전체를 저휘도 red로. 필드에서 눈 암순응 보존(콘솔·미니멀 리모컨 공유)
- **Star AF** — 화면에서 가장 밝은 별/지점에 원탭 자동 합초
- **초점 미터** — 가장 밝은 별의 실시간 선명도. MF 돌려 숫자 피크에서 정확 초점(마스크 없는 Bahtinov 대체)
- **노출 브라케팅(AEB)** — EV 보정 목록을 스텝하며 N장 촬영
- **RAW 미리보기** — `.arw`는 임베디드 JPEG을 뽑아 미리보기
- **라이브 스택** *(Stack 탭)* — 프레임을 별 정렬해 누적, SNR 향상(Average) 또는 밝은 것 누적(Lighten).
  라이브뷰 실시간 또는 저장 프레임 풀해상도 포스트스택, **lucky imaging**(선명한 상위 %만),
  **sigma-clip**(픽셀별 이상치 기각 — 위성·비행기 궤적·우주선 — 단순 평균 대신),
  **정렬 3모드** — Stars(딥스카이)·Centroid(달·행성)·ROI(박스 드래그로 특징에 고정). 진짜 **RAW 스택**
  (rawler 디모자이크 + 16bit 선형 누적)은 `--features raw` 빌드에서
- **촬영 계획** *(Plan 탭)* — 위치(또는 기기 GPS) 넣으면 지금 어떤 대상이 얼마나 높이 떠있는지.
  해·달·행성 + 밝은 별·딥스카이 카탈로그, 사용자 대상, "이거 찍기" 프리셋 넘김
